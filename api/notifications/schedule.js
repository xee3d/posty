...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const notification of notificationsToSend) {
      try {
        const sendResponse = await fetch(`${process.env.VERCEL_URL || 'http://localhost:3000'}/api/notifications/send`, {
          method: 'POST',
          headers: {
...
async function processSmartNotifications() {
  try {
    ...
    if (inactiveUsers.length > 0) {
      ...
      console.log(`📱 Sent inactive user notifications to ${inactiveUsers.length} users`);
    }

    // 개인화된 팁 알림 (랜덤하게 일부 사용자에게)
    const activeTokens = allTokens.filter(token => !inactiveUsers.includes(token));
