...
export default async function handler(req, res) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'send-test':
        // 테스트 알림 발송
        const { title = '🧪 테스트 알림', body = '포스티 테스트 메시지입니다!' } = req.query;
        
        const sendResponse = await fetch(`${req.headers.host ? `https://${req.headers.host}` : 'http://localhost:3000'}/api/notifications/send`, {
          method: 'POST',
          headers: {
      ...
      case 'check-tokens':
        ...
        for (const token of allTokens.slice(0, 10)) { // 최대 10개만
      ...
      case 'schedule-test':
        // 스케줄 테스트 (수동 실행)
        const scheduleResponse = await fetch(`${req.headers.host ? `https://${req.headers.host}` : 'http://localhost:3000'}/api/notifications/schedule?type=daily`, {
          method: 'GET',
          headers: {
