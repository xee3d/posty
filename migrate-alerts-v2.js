    ...
    files.forEach(file => {
      ...
      try {
        ...
        if (stat.isDirectory() && !file.includes('node_modules') && !file.startsWith('.')) {
          ...
          // customAlert 파일은 제외
          if (file === 'customAlert.ts' || file === 'customAlert.js' || 
              file === 'customAlert.tsx' || file === 'customAlert.jsx') {
            console.log(`${colors.yellow}Skipping:${colors.reset} ${path.relative(projectPath, filePath)}`);
            return;
          }
...
function updateFile(filePath) {
  try {
    ...
    // 이미 custom Alert import가 있는지 확인
    if (content.includes(customImport)) {
      console.log(`${colors.yellow}Already has custom Alert:${colors.reset} ${path.relative(projectPath, filePath)}`);
      return false;
    }
...
// 추가 팁
console.log(`\n${colors.yellow}Tip:${colors.reset} Run the app to check for any import errors.`);
console.log(`If you see any errors, manually check the import paths.`);
