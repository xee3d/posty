...
export function detectUserType(recentContents: string[]): string {
  if (!recentContents || recentContents.length === 0) return 'beginner';
  
  const businessKeywords = ['영업', '오픈', '이벤트', '신메뉴', '할인', '감사', '고객'];
  const influencerKeywords = ['팔로워', '좋아요', '구독', '협찬', '광고'];
  
  const allContent = recentContents.join(' ');
  
  const hasBusinessKeywords = businessKeywords.some(keyword => allContent.includes(keyword));
  const hasInfluencerKeywords = influencerKeywords.some(keyword => allContent.includes(keyword));
  
  if (hasBusinessKeywords) return 'business_manager';
  if (hasInfluencerKeywords) return 'influencer';
  if (recentContents.length < 5) return 'beginner';
...
2. ${timeContext === 'morning' ? '아침의 활기찬 느낌을' : 
   timeContext === 'evening' ? '하루를 마무리하는 편안한 느낌을' : 
   '일상적이고 편안한 느낌을'} 담아주세요
...
export function generateNaturalHashtags(
  ...
  return [...userTags, ...contentKeywords].slice(0, getOptimalHashtagCount(platform));
}
...
export function calculateEngagement(
  ...
  // 콘텐츠 요소별 가중치
  if (content.includes('?') && !content.endsWith('?')) baseScore *= 1.2; // 중간에 질문
  // 이모지 체크 (정규식 대신 간단한 방법 사용)
  const hasEmoji = /[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]/u.test(content);
  if (hasEmoji) baseScore *= 1.1; // 이모지
  if (content.length > 100 && content.length < 300) baseScore *= 1.15; // 적절한 길이
