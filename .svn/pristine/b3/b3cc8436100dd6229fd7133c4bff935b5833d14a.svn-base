...
const API_KEY = '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'; // 임시 하드코딩
...
interface PolishContentParams {
  ...
  length: 'short' | 'medium' | 'long';
  platform?: 'instagram' | 'facebook' | 'twitter' | 'linkedin' | 'blog';
  polishType?: 'spelling' | 'refine' | 'improve' | 'formal' | 'simple' | 'engaging';
}
...
class OpenAIService {
  ...
  constructor() {
    ...
    // 디버깅을 위한 로그 추가
    console.log('ENV OPENAI_API_KEY:', OPENAI_API_KEY ? `${OPENAI_API_KEY.substring(0, 10)}...` : 'undefined');
    console.log('API_KEY variable:', API_KEY ? `${API_KEY.substring(0, 10)}...` : 'undefined');
  ...
  async generateContent(params: GenerateContentParams) {
    ...
    const enhancedSystemPrompt = this.createEnhancedSystemPrompt(params.tone, params.platform);
    ...
    try {
      ...
      content = this.cleanAndOptimizeContent(content, params.platform, params.tone);
...
- 따뜻하고 진솔한 느낌을 담아주세요`;
  ...
  private cleanAndOptimizeContent(content: string, platform?: string, tone?: string): string {
    // 1. 기본 정리
    const lines = content.split('\n');
    const cleanedLines = lines.filter((line, index) => {
      const trimmedLine = line.trim();
      const isLastLine = index === lines.length - 1 || index === lines.length - 2;
    ...
    // 2. 질문형 마무리 강력하게 제거
    const questionPatterns = ['?', '여러분은', '여러분들은', '어떻게', '어떤', '무엇', '언제', '어디', '누가', '왜', '얼마나', '어떠신가요', '있으신가요', '계신가요', '하시나요'];
    const contentLines = content.split('\n').filter(line => line.trim());
    let needsNewEnding = false;
    
    // 마지막 두 줄 검사
    for (let i = contentLines.length - 1; i >= Math.max(0, contentLines.length - 2); i--) {
    ...
    if (needsNewEnding || platform === 'facebook') {
      const alternativeEndings = {
        instagram: ['오늘 하루도 감사해요 😊', '이런 순간들이 모여 행복한 하루가 되네요', '지금 이 순간을 기억하고 싶어요'],
        facebook: ['이런 경험들이 쌓여서 성장하는 것 같네요', '오늘도 의미 있는 하루였습니다', '매일 조금씩 나아지고 있어요', '이런 일상이 쌓여 행복이 되는 것 같아요', '오늘 하루도 감사한 마음으로 마무리합니다', '작은 순간들이 모여 큰 추억이 되네요', '이런 경험을 나눌 수 있어 행복합니다', '오늘의 소중한 기록을 남깁니다'],
        twitter: ['오늘도 수고했다', '이런 게 삶이다', '하루를 마무리한다'],
        linkedin: ['지속적인 성장이 핵심입니다', '매일의 작은 성장이 큰 변화를 만듭니다', '조금씩 성장하고 있습니다']
      };
      
      const selectedPlatform = platform || 'instagram';
      const endings = alternativeEndings[selectedPlatform] || alternativeEndings.instagram;
  ...
  async polishContent(params: PolishContentParams) {
    const polishGuides = {
      spelling: {
        name: '맞춤법 교정',
        instruction: '맞춤법과 문법 오류만 수정하고, 원문의 느낌은 그대로 유지해주세요.'
      },
      refine: {
        name: '문장 다듬기',
        instruction: '어색한 표현을 자연스럽게 다듬고, 문장의 흐름을 매끄럽게 만들어주세요.'
      },
      improve: {
        name: '표현 개선',
        instruction: '더 세련되고 인상적인 표현으로 바꿔주되, 원래 의미는 유지해주세요.'
      },
      formal: {
        name: '격식체 변환',
        instruction: '비즈니스나 공식적인 상황에 적합한 정중하고 격식 있는 문체로 변환해주세요. 존댓말을 사용하고 전문적인 어휘를 활용하세요.'
      },
      simple: {
        name: '쉽게 풀어쓰기',
        instruction: '누구나 이해하기 쉽도록 간단한 단어와 짧은 문장으로 바꿔주세요. 전문용어는 쉼운 말로 풀어서 설명해주세요.'
      },
      engaging: {
        name: '매력적으로 만들기',
        instruction: '독자의 관심을 끌 수 있도록 흥미롭고 생동감 있게 만들어주세요. 감정을 자극하는 표현과 생생한 묘사를 추가하세요.'
      }
    };
...
4. 해시태그가 있다면 유지하되, 더 나은 해시태그로 교체할 수 있습니다`;
    ...
    const userPrompt = `다음 글을 ${toneGuides[params.tone]} ${lengthGuides[params.length]} 다듬어주세요:\n\n${params.text}\n\n수정된 글과 함께 어울리는 해시태그 5-7개를 추가해주세요.`;
    
    console.log('Polishing content with OpenAI:', { tone: params.tone, length: params.length });
...
- 해시태그는 포함하지 마세요`
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (foundEmptyLine && line.length > 10) {
          // 번호나 불리트 제거
          const cleanedLine = line.replace(/^[1-3][\.)\:]\s*|^[-•]\s*/, '').trim();
          if (cleanedLine) {
...
- 플랫폼별 특성에 맞는 고유한 표현을 사용하세요`;
  ...
  private createUserPrompt(params: GenerateContentParams): string {
    ...
    const platformInstruction = endingStyles[params.platform] || endingStyles.instagram[randomIndex % 10];
...
중요: 반드시 위에 제시된 "${params.prompt}" 주제에 대해서만 작성하고, 다른 내용은 절대 쓰지 마세요.
해시태그는 글 끝에 포함해주세요.`;
  ...
  private extractHashtags(content: string, platform?: string): string[] {
    ...
    if (uniqueTags.length < targetCount) {
      console.log(`Hashtags needed: ${targetCount}, found: ${uniqueTags.length}`);
      
      // 기본 추가 태그
      const additionalTags = [
        '일상', '데일리', '오늘', '추천', '소통', '맞팔', '좋아요', 
        '팔로우', '일상스타그램', '인스타그램', '스타그램', '감성',
        '행복', '공유', '기록', '추억', '순간', '하루', '감사'
      ];
  ...
  private extractObjects(description: string): string[] {
    const keywords = ['커피', '라떼', '카페', '테이블', '창문', '음식', '디저트', '꽃', '하늘', '바다'];
  ...
  private detectMood(description: string): string {
    ...
    if (description.includes('활기') || description.includes('즐거')) return 'cheerful';
    return 'positive';
  }
}
