...
export const useOptimizedTheme = () => {
  ...
  // 그라데이션
  const gradients = isDark ? DARK_GRADIENTS : {
  ...
  // 그림자
  const shadows = isDark ? DARK_SHADOWS : {
    small: {
  ...
  // 카드 스타일
  const cardStyles = isDark ? DARK_CARD_STYLES : {
    default: {
  ...
  // 컴포넌트 스타일
  const componentStyles = isDark ? DARK_COMPONENT_STYLES : {
    button: {
      primary: {
