...
import { pushNotificationService, NotificationPayload } from '../services/notification/pushNotificationService';
...
export interface UsePushNotificationsReturn {
  ...
  sendTestNotification: (type: 'mission' | 'trend' | 'token' | 'achievement') => Promise<void>;
...
export const usePushNotifications = (): UsePushNotificationsReturn => {
   ...
   * 테스트 알림 전송
   */
  const sendTestNotification = useCallback(async (type: 'mission' | 'trend' | 'token' | 'achievement'): Promise<void> => {
  ...
  const scheduleSmartNotifications = useCallback(async (): Promise<void> => {
    if (!state.isInitialized || !state.hasPermission) {
      console.log('📱 Cannot schedule notifications: not initialized or no permission');
      return;
    }
