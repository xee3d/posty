...
class SubscriptionSecurityManager {
  ...
  private async getDeviceFingerprint(): Promise<string> {
    try {
      ...
      console.error(
        "[SubscriptionSecurity] Failed to get device fingerprint:",
        error
      );
   ...
   * 구독 정보 서명 생성
   */
  private generateSubscriptionSignature(
    subscription: Omit<SecureSubscription, "signature">
  ): string {
   ...
   * 구독 정보 검증
   */
  private verifySubscriptionSignature(
  ...
  async loadSecureSubscription(): Promise<SecureSubscription | null> {
    try {
      ...
      if (!this.verifySubscriptionSignature(subscription)) {
        console.error(
          "[SubscriptionSecurity] Invalid subscription signature - possible tampering"
        );
      ...
      if (subscription.deviceId !== this.deviceId) {
        console.error(
          "[SubscriptionSecurity] Device mismatch - possible data migration"
        );
        await this.clearSecureSubscription();
        return null;
      }
      ...
      console.log(
        "[SubscriptionSecurity] Subscription loaded and verified:",
        subscription.plan
      );
      return subscription;
    } catch (error) {
      console.error(
        "[SubscriptionSecurity] Failed to load subscription:",
        error
      );
      await this.clearSecureSubscription();
      return null;
  ...
  async validateSubscription(): Promise<SubscriptionValidationResult> {
    try {
      ...
      if (subscription.expiresAt) {
        ...
        if (now > expiryDate) {
          console.log(
            "[SubscriptionSecurity] Subscription expired, reverting to free"
          );
      ...
      if (daysSinceVerification > 7) {
        console.warn(
          "[SubscriptionSecurity] Subscription verification expired, server validation required"
        );
        // 서버 검증이 구현되면 여기서 호출
  ...
  async detectSuspiciousActivity(): Promise<{
    ...
    try {
      ...
      console.error(
        "[SubscriptionSecurity] Suspicious activity detection failed:",
        error
      );
      return {
        issuspicious: true,
  ...
  async clearSecureSubscription(): Promise<void> {
    try {
      ...
      console.error(
        "[SubscriptionSecurity] Failed to clear subscription data:",
        error
      );
    }
  }
