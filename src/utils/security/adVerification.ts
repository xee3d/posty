...
class AdVerificationManager {
   ...
   * 광고 시청 완료 검증
   */
  async verifyAdCompletion(earnedReward: number = 2): Promise<AdVerificationResult> {
    try {
      ...
      const signature = await this.generateAdVerificationSignature(verificationData);
   ...
   * 광고 검증 서명 생성
   */
  private async generateAdVerificationSignature(data: AdVerificationRequest): Promise<string> {
  ...
  private async recordAdView(
    ...
    // 마지막 광고 시간 기록
    await AsyncStorage.setItem('@posty_last_ad_time', data.timestamp.toString());
  }

  /**
   * 광고 메트릭 업데이트
   */
  private async updateAdMetrics(viewTime: number, rewarded: boolean): Promise<void> {
    const key = '@posty_ad_metrics';
    const existingMetrics = await AsyncStorage.getItem(key);
    const metrics: AdMetrics = existingMetrics ? JSON.parse(existingMetrics) : {
      totalShown: 0,
      totalRewarded: 0,
      averageViewTime: 0,
   ...
   * 의심스러운 활동 로깅
   */
  private async logSuspiciousActivity(activity: string, details: any): Promise<void> {
  ...
  async getAdStatistics(): Promise<{
    ...
    return {
      ...
      successRate: metrics.totalShown > 0 ? metrics.totalRewarded / metrics.totalShown : 0,
