...
export const optimizeForPlatform = (
  ...
  console.log(
    `[platformOptimizer] Original content length: ${originalContent.length}`
  );
  ...
  try {
    ...
    const contentWithoutHashtags = originalContent
      .replace(hashtagRegex, "")
      .trim();

    let optimizedContent = contentWithoutHashtags;
    let optimizedHashtags =
      originalHashtags && originalHashtags.length > 0
    ...
    console.log(
      `[platformOptimizer] Optimized content length: ${optimizedContent.length}`
    );
...
const transformToInstagram = (content: string, tone: string): string => {
  ...
  if (sentences.length > 0 && emojiMap[tone]) {
    const emoji =
      emojiMap[tone][Math.floor(Math.random() * emojiMap[tone].length)];
    sentences[0] += ` ${emoji}`;
  ...
  if (isExtraLong) {
    ...
    if (transformed.length > maxLength) {
      ...
      for (const sentence of sentences) {
        if (trimmed.length + sentence.length + 2 <= maxLength - 100) {
          // 마무리 공간
...
const transformToFacebook = (content: string, tone: string): string => {
  ...
  if (shouldAddStarter && tone !== "professional" && tone !== "minimalist") {
    ...
    const hasSkipKeyword = skipKeywords.some((keyword) =>
      firstSentence.includes(keyword)
    );

    if (!hasSkipKeyword) {
      // 매우 자연스러운 연결어만 사용
      const naturalConnectors = ["그런데 말이에요.\n\n", "생각해보니까.\n\n"];
      transformed =
        naturalConnectors[
  ...
  if (shouldAddEnding) {
    ...
    if (
      transformed.trim().endsWith("?") ||
      transformed.trim().endsWith("!") ||
      transformed.includes("그래서") ||
      transformed.includes("그런데")
    ) {
      return transformed;
    }
    ...
    if (transformed.length > maxContentLength) {
      if (isExtraLong) {
        ...
        if (sentences.length === 0) {
          transformed =
            transformed.substring(0, 200) + "...\n\n🧵 (스레드에서 이어집니다)";
        } else {
          // 첫 1-2문장으로 핵심 요약
          let summary = "";
          for (let i = 0; i < Math.min(2, sentences.length); i++) {
            const sentence = sentences[i].trim();
            if (summary.length + sentence.length + 2 <= 180) {
              // 스레드 안내 공간 확보
