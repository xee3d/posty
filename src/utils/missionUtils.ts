  ...
  try {
    ...
    if (missionsData) {
      ...
      if (missionIndex !== -1) {
        ...
        missions[missionIndex].completed = progress >= missions[missionIndex].target;
...
export const debugMissionData = async () => {
  try {
    ...
    const completedAchievements = await AsyncStorage.getItem('completed_achievements');
    
    console.log('=== Mission Debug Data ===');
    console.log('Missions:', missionsData ? JSON.parse(missionsData) : 'No data');
    console.log('Last Reset:', lastReset || 'Never');
    console.log('Completed Achievements:', completedAchievements ? JSON.parse(completedAchievements) : []);
    console.log('========================');
  } catch (error) {
