...
class CostAnalyzer {
  ...
  getCostSavingTips() {
    ...
    if (
      efficiency.length > 0 &&
      efficiency[0].model !== "deepseek/deepseek-chat"
    ) {
      tips.push({
        title: "무료 모델 활용",
  ...
  generateMonthlyReport() {
    ...
    return {
      ...
      costBreakdown: {
        ...
        byFeature: {
          textGeneration: Object.entries(stats.modelBreakdown).reduce(
            (sum, [_, data]) => sum + data.cost * 0.7,
            0
          ), // 추정치
          imageAnalysis: Object.entries(stats.modelBreakdown).reduce(
            (sum, [_, data]) => sum + data.cost * 0.3,
            0
          ), // 추정치
        },
      },
