...
export const ModernSubscriptionScreen: React.FC<SubscriptionScreenProps> = ({
  ...
  const [realSubscriptionPlan, setRealSubscriptionPlan] = useState<'free' | 'pro'>('free');
  ...
  const calculateTokenChange = (newPlan: 'pro') => {
    let tokenChange = 0;
    let description = '';

    if (subscriptionPlan === 'free' && newPlan === 'pro') {
      tokenChange = 9999;
  ...
  const handleSubscribe = async (planKey?: 'free' | 'pro') => {
    ...
    // 다운그레이드 체크 (Pro → Free만 체크)
    const isDowngrade = subscriptionPlan === 'pro' && targetPlan !== 'pro';
  ...
  const renderPlanCard = (planKey: 'free' | 'pro') => {
    ...
    // 다운그레이드 체크 (Pro → Free만)
    const isDowngrade = subscriptionPlan === 'pro' && planKey !== 'pro';
            ...
            <View style={styles.benefitsSection}>
              ...
              <View style={styles.benefitCard}>
                ...
                <View style={styles.benefitContent}>
                  <Text style={styles.benefitTitle}>무제한 토큰</Text>
                  <Text style={styles.benefitDesc}>
                  PRO 플랜은 무제한 토큰을 제공하여 걱정 없이 콘텐츠를 생성할 수 있습니다
                  </Text>
                </View>
              </View>
