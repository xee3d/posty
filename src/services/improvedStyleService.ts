...
class ImprovedStyleService {
  ...
  async analyzeUserStyle(): Promise<StyleAnalysis> {
    try {
      ...
      // 추천사항 생성
      const recommendations = this.generateRecommendations(dominantStyle, consistency, diversity);
      
      // 강점과 개선점
      const { strengths, improvements } = this.analyzeStrengthsAndImprovements(posts, styleScores);
  ...
  private calculateStyleScores(posts: any[]): Record<string, number> {
    ...
    STYLE_TEMPLATES.forEach(template => {
      ...
      posts.forEach(post => {
        // 길이 매칭
        if (template.id === 'minimalist' && post.content.length <= 50) score += 10;
        if (template.id === 'storyteller' && post.content.length >= 200) score += 10;
        if (template.id === 'trendsetter' && post.content.length >= 100 && post.content.length <= 150) score += 10;
        
        // 키워드 매칭
        template.characteristics.keywords.forEach(keyword => {
          if (post.content.includes(keyword)) score += 5;
        });
        
        // 이모지 사용 패턴
        const emojiCount = (post.content.match(/[\u{1F300}-\u{1F9FF}]/gu) || []).length;
        if (template.id === 'minimalist' && emojiCount <= 2) score += 5;
        if (template.id === 'trendsetter' && emojiCount >= 3) score += 5;
        
        // 해시태그 패턴
        if (template.id === 'trendsetter' && post.hashtags.length >= 5) score += 5;
        if (template.id === 'minimalist' && post.hashtags.length <= 3) score += 5;
      });
  ...
  private analyzeDiversity(posts: any[]): number {
    ...
    // 다양성 점수 계산
    const hashtagDiversity = Math.min(uniqueHashtags / posts.length * 100, 100);
    const categoryDiversity = Math.min(uniqueCategories / 5 * 100, 100);
    const toneDiversity = Math.min(uniqueTones / 4 * 100, 100);
    
    return Math.round((hashtagDiversity + categoryDiversity + toneDiversity) / 3);
  }
  ...
  private analyzeGrowth(posts: any[]): any {
    ...
    const avgLengthTrend = Object.values(weeklyLengths).map(lengths => 
      lengths.reduce((a, b) => a + b, 0) / lengths.length
    );
    
    const uniqueHashtags = new Set(posts.flatMap(p => p.hashtags)).size;
    const hashtagDiversity = Math.round(uniqueHashtags / Math.max(posts.length, 1) * 100);
  ...
  // 추천사항 생성
  private generateRecommendations(dominantStyle: string, consistency: number, diversity: number): string[] {
  ...
  // 강점과 개선점 분석
  private analyzeStrengthsAndImprovements(posts: any[], styleScores: Record<string, number>): {
    strengths: string[];
    improvements: string[];
  } {
    ...
    // 포스팅 빈도
    const avgPostsPerWeek = posts.length / Math.max(1, this.getWeeksBetween(
  ...
  async startChallenge(challengeId: string): Promise<boolean> {
    try {
      ...
      await AsyncStorage.setItem(this.STORAGE_KEYS.STYLE_CHALLENGES, JSON.stringify(activeChallenge));
      return true;
    } catch (error) {
  ...
  async updateChallengeProgress(postId: string): Promise<void> {
    try {
      const challengeData = await AsyncStorage.getItem(this.STORAGE_KEYS.STYLE_CHALLENGES);
      ...
      if (this.validateChallengePost(post, challenge)) {
        ...
        await AsyncStorage.setItem(this.STORAGE_KEYS.STYLE_CHALLENGES, JSON.stringify(challenge));
      }
    } catch (error) {
  ...
  private async awardAchievement(rewards: any): Promise<void> {
    try {
      const achievementsData = await AsyncStorage.getItem(this.STORAGE_KEYS.STYLE_ACHIEVEMENTS);
  ...
  async getAchievements(): Promise<any[]> {
    try {
      const data = await AsyncStorage.getItem(this.STORAGE_KEYS.STYLE_ACHIEVEMENTS);
  ...
  private async saveAnalysis(analysis: StyleAnalysis): Promise<void> {
    try {
      await AsyncStorage.setItem(this.STORAGE_KEYS.STYLE_ANALYSIS, JSON.stringify({
        ...analysis,
        timestamp: new Date()
      }));
    } catch (error) {
