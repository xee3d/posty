...
class SubscriptionService {
    ...
    try {
      ...
      if (savedSubscription) {
        ...
        if (this.currentSubscription?.expiresAt) {
          this.currentSubscription.expiresAt = new Date(this.currentSubscription.expiresAt);
        }
        if (this.currentSubscription?.lastResetDate) {
          this.currentSubscription.lastResetDate = new Date(this.currentSubscription.lastResetDate);
        }
  ...
  async setSubscription(plan: 'free' | 'premium' | 'pro') {
    const now = new Date();
    const expiresAt = plan === 'free' ? undefined : new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000); // 30일
  ...
  private async checkSubscriptionExpiry() {
    if (!this.currentSubscription || !this.currentSubscription.expiresAt) return;
  ...
  private async saveSubscription() {
    ...
    try {
      await AsyncStorage.setItem(SUBSCRIPTION_KEY, JSON.stringify(this.currentSubscription));
    } catch (error) {
  ...
  // 특정 기능 사용 가능 여부 확인
  async canUseFeature(feature: 'removeAds' | 'premiumAI' | 'unlimitedGeneration'): Promise<boolean> {
    const subscription = await this.getSubscription();
    const plan = SUBSCRIPTION_PLANS[subscription.plan];
