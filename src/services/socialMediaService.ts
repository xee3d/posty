...
class SocialMediaService {
  ...
  // 액세스 토큰 저장
  async saveAccessToken(platform: 'instagram' | 'facebook', token: string): Promise<void> {
    try {
      const tokens = await this.getAccessTokens();
      tokens[platform] = token;
      await AsyncStorage.setItem(this.STORAGE_KEYS.ACCESS_TOKENS, JSON.stringify(tokens));
    } catch (error) {
  ...
  async getAccessTokens(): Promise<AccessTokens> {
    try {
      const tokensJson = await AsyncStorage.getItem(this.STORAGE_KEYS.ACCESS_TOKENS);
  ...
  async savePostMapping(localPostId: string, platform: 'instagram' | 'facebook', mediaId: string): Promise<void> {
    try {
      ...
      await AsyncStorage.setItem(this.STORAGE_KEYS.POST_MAPPINGS, JSON.stringify(mappings));
    } catch (error) {
  ...
  // 게시물 ID 매핑 가져오기
  async getPostMappings(): Promise<Record<string, { platform: string; mediaId: string }>> {
    try {
      const mappingsJson = await AsyncStorage.getItem(this.STORAGE_KEYS.POST_MAPPINGS);
  ...
  async syncAllPostInsights(): Promise<void> {
    try {
      ...
      console.log('Synced insights:', results.filter(r => r !== null));
    } catch (error) {
