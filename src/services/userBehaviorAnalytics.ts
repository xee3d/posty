...
class UserBehaviorAnalytics {
  ...
  async analyzeUserWritingPatterns(): Promise<void> {
    try {
      ...
      // 선호도 업데이트
      behaviorData.writingPatterns.preferredTimes = Object.entries(timePreferences)
        .sort(([,a], [,b]) => b - a)
        .slice(0, 3)
        .map(([hour]) => parseInt(hour));
        
      behaviorData.writingPatterns.preferredDays = Object.entries(dayPreferences)
        .sort(([,a], [,b]) => b - a)
        .slice(0, 2)
        .map(([day]) => parseInt(day));
        
      behaviorData.writingPatterns.averageLength = Math.round(totalLength / posts.length);
      
      behaviorData.writingPatterns.favoriteTopics = Object.entries(topicFrequency)
        .sort(([,a], [,b]) => b - a)
        .slice(0, 5)
        .map(([topic]) => topic);
      ...
      behaviorData.writingPatterns.favoritePlatforms = Object.entries(platformFrequency)
        .sort(([,a], [,b]) => b - a)
        .slice(0, 2)
  ...
  private categorizeHashtag(hashtag: string): string {
    const categories = {
      'food': ['맛집', '먹스타그램', '음식', '커피', '카페', '요리', '레시피', '맛있다', '점심', '저녁', '아침', '간식', '디저트'],
      'lifestyle': ['일상', '라이프스타일', '힐링', '여유', '소소한행복', '오늘하루', '하루', '데일리', '루틴', '취미'],
      'travel': ['여행', '여행스타그램', '관광', '휴가', '바다', '산', '나들이', '드라이브', '출장', '여행지'],
      'fashion': ['패션', '옷', '스타일', 'ootd', '코디', '쇼핑', '옷스타그램', '패션스타그램'],
      'fitness': ['운동', '헬스', '요가', '다이어트', '건강', '홈트', '피트니스', '런닝', '헬스장'],
      'work': ['업무', '일', '회사', '미팅', '프로젝트', '직장', '사무실', '워라밸', '개발', '공부'],
      'entertainment': ['영화', '드라마', '음악', '게임', '독서', '넷플릭스', 'youtube', '유튜브', '책'],
      'social': ['친구', '가족', '모임', '파티', '데이트', '만남', '약속', '동료', '연인'],
      'photo': ['셀카', '사진', '포토', '인스타', 'selfie', '찰칵', '스냅', '기념'],
      'mood': ['행복', '기분좋다', '힘들다', '피곤', '신나', '우울', '감사', '좋다']
    };
    
    const lowerHashtag = hashtag.toLowerCase();
    
    for (const [category, keywords] of Object.entries(categories)) {
      if (keywords.some(keyword => lowerHashtag.includes(keyword.toLowerCase()))) {
        return category;
      }
    }
  ...
  async recordRecommendationClick(recommendationId: string): Promise<void> {
    try {
      const behaviorData = await this.getBehaviorData();
      behaviorData.interactionPatterns.clickedRecommendations.push(recommendationId);
  ...
  private async cleanupGeneralData(): Promise<void> {
    try {
      ...
      if (behaviorData.writingPatterns.favoriteTopics.includes('general')) {
        behaviorData.writingPatterns.favoriteTopics = behaviorData.writingPatterns.favoriteTopics
  ...
  async generatePersonalizedRecommendations(): Promise<PersonalizedRecommendation[]> {
    try {
      ...
      const validTopics = behaviorData.writingPatterns.favoriteTopics.filter(topic => topic !== 'general');
      if (validTopics.length > 0) {
        ...
        recommendations.push({
          ...
          content: `${this.getTopicDisplayName(topTopic)} 관련 글을 자주 쓰시네요!\n새로운 ${this.getTopicDisplayName(topTopic)} 이야기는 어떠세요?`,
          actionText: '써보기',
          actionPayload: { 
