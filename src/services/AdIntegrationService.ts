...
export type AdPosition = 'feed' | 'home_top' | 'home_middle' | 'post_list' | 'after_action' | 'trend' | 'style' | 'ai_write';
...
class AdIntegrationService {
  ...
  trackUserAction(actionType: string): void {
    this.actionCounter++;
    console.log(`Action tracked: ${actionType}, Counter: ${this.actionCounter}`);
    
    // 주요 액션인 경우에만 카운트
    const majorActions = ['create_post', 'save_post', 'share_post', 'navigate_tab'];
  ...
  shouldShowNativeAd(position: number, context?: string): boolean {
    ...
    switch (context) {
      case 'home':
        if (position === 0) return timeBasedValue < 5; // 홈 상단 50%
        if (position === 10) return timeBasedValue < 3; // 홈 하단 30%
        return this.config.nativeAdPositions.includes(position);
      
      case 'trend':
        if (position === 1) return timeBasedValue < 7; // 트렌드 화면 70%
        return false;
        
      case 'style':
        if (position === 2) return timeBasedValue < 6; // 스타일 화면 60%
        return false;
        
      case 'ai_write':
        if (position === 5) return timeBasedValue < 4; // AI 글쓰기 화면 40%
        return false;
  ...
  getAdLayoutType(position: number): 'feed' | 'card' | 'banner' | 'inline' | 'fullwidth' {
    if (position === 0) return 'fullwidth'; // 첫 번째 광고는 전체 너비
    if (position % 10 === 0) return 'banner'; // 10번째마다 배너
    if (position % 5 === 0) return 'card'; // 5번째마다 카드
    if (position % 7 === 0) return 'inline'; // 7번째마다 인라인
  ...
  getUserSegment(): 'high_value' | 'medium_value' | 'low_value' {
    ...
    if (random < 0.2) return 'high_value';
    if (random < 0.6) return 'medium_value';
    return 'low_value';
  }
