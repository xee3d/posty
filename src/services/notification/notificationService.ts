...
class NotificationService {
  ...
  async sendSmartNotification(
    ...
    try {
      if (
        pushNotificationService &&
        pushNotificationService.sendSmartNotification
      ) {
        return await pushNotificationService.sendSmartNotification(type);
      } else {
        console.log(
          `📱 Smart notification (${type}) not available in fallback mode`
        );
        return false;
      }
    } catch (error) {
  ...
  async subscribeToTopic(topic: string) {
    console.log(
      `📱 Topic subscription not supported in current implementation: ${topic}`
    );
  }

  async unsubscribeFromTopic(topic: string) {
    console.log(
      `📱 Topic unsubscribe not supported in current implementation: ${topic}`
    );
  }

  async refreshToken() {
    console.log(
      "📱 Token refresh handled automatically by pushNotificationService"
    );
    return this.getDeviceToken();
  }
}
