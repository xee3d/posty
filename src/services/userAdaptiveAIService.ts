...
- 자연스러운 이모지 사용`,
      ...
      business_manager: `당신은 ${
        userProfile.businessType || "비즈니스"
      }의 SNS를 관리합니다.
특징:
- 브랜드 아이덴티티 일관성
- 고객과의 친근한 소통
...
- 지역 커뮤니티와의 연결`,
  ...
  private getUserTypeHashtags(
    ...
    const strategy =
      hashtagStrategies[userProfile.type] || hashtagStrategies.casual_user;
...
4. ${
      shouldAddEngagement
        ? `"${engagementElements[0]}" 같은 참여 유도 요소를 자연스럽게 포함하세요`
        : "과한 참여 유도는 피하세요"
    }
5. 문장이 너무 매끄럽거나 완벽하면 오히려 부자연스럽습니다
6. 실제 ${
      userProfile.type === "business_manager" ? "사장님이" : "사람이"
    } 쓴 것처럼 따뜻하고 진솔하게
