...
2. 실제 ${this.getAgeGroupDescription(userProfile.ageGroup)} ${this.getGenderDescription(userProfile.gender)}이(가) 쓴 것처럼 자연스럽게
...
`;
  ...
  // 페르소나 구축
  private buildPersona(profile: DetailedUserProfile, imageContext?: string): string {
    ...
    if (profile.interests && profile.interests.length > 0) {
      persona += ` 주요 관심사는 ${profile.interests.slice(0, 3).join(', ')} 등입니다.`;
    }
    
    // 특별한 컨텍스트 추가
    if (imageContext === 'baby_photo' && profile.parentType) {
      persona += ` 아이를 사랑하는 ${profile.parentType === 'mother' ? '엄마' : '아빠'}의 마음으로 글을 씁니다.`;
    } else if (imageContext === 'baby_photo' && profile.familyRole === 'grandparent') {
  ...
  // 가족 컨텍스트 프롬프트
  private getFamilyContextPrompt(profile: DetailedUserProfile, imageContext?: string): string {
  ...
  // 해시태그 전략
  private getHashtagStrategy(profile: DetailedUserProfile, platform: string): string {
    ...
    if (profile.parentType) {
      hashtags.push(profile.parentType === 'mother' ? '엄마스타그램' : '아빠스타그램');
      if (profile.childrenAge === 'baby') hashtags.push('육아스타그램', '아기스타그램');
    }
    ...
    return `추천 해시태그: ${hashtags.join(' ')} (플랫폼과 상황에 맞게 5-7개 선택)`;
  }
  
  // 특별 지침
  private getSpecialInstructions(profile: DetailedUserProfile, imageContext?: string): string {
    const instructions: string[] = [];
    
    // 연령대별 특별 지침
    if (profile.ageGroup === '10s' || profile.ageGroup === '20s') {
      instructions.push('이모지와 ㅋㅋㅋ, ㅠㅠ 같은 표현을 자연스럽게 사용하세요');
    } else if (profile.ageGroup === '50s' || profile.ageGroup === '60s+') {
      instructions.push('이모지는 적당히, 진솔하고 따뜻한 표현을 사용하세요');
    }
    
    // 성별 특별 지침
    if (profile.gender === 'female' && (profile.ageGroup === '20s' || profile.ageGroup === '30s')) {
  ...
  detectImageContext(imageDescription: string): string {
    // 키워드 기반 간단한 감지
    if (imageDescription.includes('아기') || imageDescription.includes('baby')) {
      return 'baby_photo';
    } else if (imageDescription.includes('음식') || imageDescription.includes('food')) {
      return 'food';
    } else if (imageDescription.includes('여행') || imageDescription.includes('travel')) {
      return 'travel';
    } else if (imageDescription.includes('카페') || imageDescription.includes('coffee')) {
      return 'cafe';
    }
