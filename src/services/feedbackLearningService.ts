...
export class FeedbackLearningService {
  ...
  private analyzeEditPatterns(
    ...
    // 이모지 사용 분석
    const originalEmojis = (original.match(/[\u{1F300}-\u{1F9FF}]/gu) || [])
      .length;
  ...
  async getPersonalizedPrompt(
    ...
    const goodExamples = feedback
      .filter(
        (f) => f.platform === platform && f.tone === tone && f.userRating >= 4
      )
    ...
    const badExamples = feedback
      .filter(
        (f) => f.platform === platform && f.tone === tone && f.userRating <= 2
      )
      .slice(0, 2);
  ...
  async getRecommendedSettings(): Promise<{ platform: string; tone: string }> {
    ...
    Object.entries(preferences.platforms).forEach(
