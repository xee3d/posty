...
class DailyTipsService {
  ...
  private performanceTips = [
    { emoji: '📈', label: '지난주 대비', format: (growth: number) => `+${growth}%`, subtext: '와! 대단해요!' },
    { emoji: '🚀', label: '이번 달 성장', format: (growth: number) => `+${growth}%`, subtext: '급성장 중!' },
    { emoji: '💯', label: '참여율', format: (rate: number) => `${rate}%`, subtext: '최고 수준!' },
    { emoji: '🎯', label: '목표 달성률', format: (rate: number) => `${rate}%`, subtext: '거의 다 왔어요!' },
    { emoji: '⭐', label: '평균 좋아요', format: (likes: number) => `${likes}개`, subtext: '인기 만점!' },
    { emoji: '📊', label: '주간 활동', format: (posts: number) => `${posts}개`, subtext: '꾸준해요!' },
    { emoji: '🎉', label: '최고 기록', format: (record: number) => `${record}개`, subtext: '신기록!' },
    { emoji: '🔥', label: '연속 게시', format: (days: number) => `${days}일째`, subtext: '대단해요!' },
  ];

  // 최적 시간대 팁
  private timeTips = [
    { emoji: '⏰', label: '최고 시간', times: ['오전 7시', '오후 1시', '오후 7시', '오후 9시'] },
    { emoji: '🌅', label: '아침 추천', times: ['오전 7-9시', '오전 6-8시', '오전 8-10시'] },
    { emoji: '🌙', label: '저녁 추천', times: ['오후 7-9시', '오후\n8-10시', '오후\n6-8시'] },
    { emoji: '☀️', label: '점심 추천', times: ['오후 12-1시', '오전 11-12시', '오후 1-2시'] },
    { emoji: '🕐', label: '피크 타임', times: ['오후 2시', '오후 5시', '오후 8시'] },
    { emoji: '📅', label: '주말 추천', times: ['오전 10시', '오후 3시', '오후 6시'] },
  ];

  // 트렌딩 키워드 팁
  private trendingTips = [
    { emoji: '🔥', label: '핫 키워드', keywords: ['#일상', '#주말', '#맛집', '#카페', '#운동', '#여행', '#공부'] },
    { emoji: '📍', label: '지역 인기', keywords: ['#서울', '#부산', '#제주', '#강남', '#홍대', '#성수동', '#을지로'] },
    { emoji: '🎨', label: '스타일 추천', keywords: ['#미니멀', '#감성', '#빈티지', '#모던', '#힙스터', '#코지', '#내추럴'] },
    { emoji: '💡', label: '아이디어', keywords: ['#꿀팁', '#추천', '#리뷰', '#후기', '#정보', '#노하우', '#팁'] },
    { emoji: '🌈', label: '무드 추천', keywords: ['#힐링', '#행복', '#소확행', '#위로', '#설렘', '#추억', '#감사'] },
    { emoji: '🎯', label: '타겟 추천', keywords: ['#20대', '#직장인', '#학생', '#엄마', '#자취생', '#신혼', '#댕댕이'] },
  ];
  ...
  async calculateUserStats(): Promise<UserStats> {
    try {
      ...
      // 인기 해시태그
      const allHashtags = savedContents.flatMap(content => content.hashtags || []);
      ...
      return {
        ...
        topHashtags: topHashtags.length > 0 ? topHashtags : ['#일상', '#주말', '#카페'],
  ...
  async getDailyTips(): Promise<DailyTip[]> {
    ...
    // 1. 성과 팁 (랜덤)
    const perfTip = this.performanceTips[Math.floor(Math.random() * this.performanceTips.length)];
    let value: number;
    ...
    if (isNaN(value) || value === undefined || value === null || (stats.totalPosts === 0 && value === 0)) {
      // 데이터가 없을 때는 기본 팁으로 변경
      const defaultTip = this.generalTips[Math.floor(Math.random() * this.generalTips.length)];
      tips.push({
        id: '1',
        category: 'tip',
        ...defaultTip,
    ...
    // 2. 시간대 팁
    const timeTip = this.timeTips[Math.floor(Math.random() * this.timeTips.length)];
    ...
    if (timeTip.label === '최고 시간' && stats.totalPosts > 0) {
      ...
      // 랜덤 시간 선택
      timeValue = timeTip.times[Math.floor(Math.random() * timeTip.times.length)];
    }
    ...
    if (Math.random() > 0.5 && stats.topHashtags.length > 0) {
      // 트렌딩 키워드
      const trendTip = this.trendingTips[Math.floor(Math.random() * this.trendingTips.length)];
      ...
      // 일반 팁
      const generalTip = this.generalTips[Math.floor(Math.random() * this.generalTips.length)];
      tips.push({
        id: '3',
        category: 'tip',
        ...generalTip,
