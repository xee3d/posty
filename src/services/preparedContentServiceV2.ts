...
class PreparedContentService {
  ...
  private async analyzeWritingStyle(contents: any[]): Promise<WritingStyle> {
    ...
    // 이모지 사용량 분석
    const emojiCounts = contents.map(
      (c) =>
        (
          c.generatedContent?.match(
            /[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]/gu
          ) || []
        ).length
    );
    const avgEmojiCount =
      emojiCounts.reduce((a, b) => a + b, 0) / contents.length;

    // 해시태그 평균 개수
    const avgHashtagCount =
      contents.map((c) => c.hashtags?.length || 0).reduce((a, b) => a + b, 0) /
      contents.length;
  ...
  private analyzePostingPatterns(contents: any[]): PostingPattern {
    ...
    const bestHour =
      Object.entries(hourCounts).sort(([, a], [, b]) => b - a)[0]?.[0] || "19";
    ...
    const daysSinceFirst =
      contents.length > 0
        ? (Date.now() -
            new Date(contents[contents.length - 1].timestamp).getTime()) /
          (1000 * 60 * 60 * 24)
        : 30;

    const postsPerWeek = (uniqueDates / daysSinceFirst) * 7;
    const frequency =
      postsPerWeek > 5 ? "daily" : postsPerWeek > 2 ? "weekly" : "occasional";

    // 인기 주제 분석 (해시태그 기반)
    const popularTopics = this.getFavoriteHashtags(contents).slice(0, 5);

    return {
      bestTime: `${parseInt(bestHour) < 12 ? "오전" : "오후"} ${
        parseInt(bestHour) % 12 || 12
      }시`,
      frequency,
      popularTopics,
    };
  ...
  // 개인화된 콘텐츠 생성
  private async generatePersonalizedContents(
    context: UserContext
  ...
  // 사진 기반 콘텐츠 생성
  private createPhotoBasedContent(
    photoType: string,
    context: UserContext
  ): PreparedContent {
    const photoContents: Record<string, PreparedContent> = {
      "카페 사진": {
        ...
        emoji: "☕",
        title: "카페 사진 캡션",
        content: `${this.getTimeGreeting()}의 여유로운 카페타임${
          context.writingStyle.emojiUsage !== "minimal" ? " ☕" : ""
        }. 따뜻한 커피 한 잔과 함께하는 ${this.getDayOfWeek()}`,
        hashtags: [
          ...context.favoriteHashtags.slice(0, 3),
          "카페",
          "커피타임",
          this.getDayOfWeek(),
        ],
      ...
      "음식 사진": {
        ...
        content: `오늘의 맛있는 발견${
          context.writingStyle.emojiUsage !== "minimal" ? " 🤤" : ""
        }! ${this.getRandomFoodComment()} 여러분도 맛있는 하루 보내세요`,
        hashtags: [
          ...context.favoriteHashtags.slice(0, 3),
          "맛집",
          "먹스타그램",
  ...
  // 일정 기반 콘텐츠 생성
  private createEventBasedContent(
    event: CalendarEvent,
    context: UserContext
  ): PreparedContent {
    const eventContents: Record<string, PreparedContent> = {
      "친구 만남": {
        ...
        emoji: "👥",
        title: "친구와의 시간",
        content: `오랜만에 만나는 친구들과의 시간이 기대돼요${
          context.writingStyle.emojiUsage !== "minimal" ? " 🥰" : ""
        }! ${event.location}에서 즐거운 추억 만들어야지`,
    ...
    return key
      ? eventContents[key]
      : this.getDefaultEventContent(event, context);
  }
  ...
  private getDefaultEventContent(
    event: CalendarEvent,
    context: UserContext
  ): PreparedContent {
    return {
      ...
      emoji: "📅",
      title: event.title,
      content: `${event.title}이(가) 있는 날! ${
        event.location ? `${event.location}에서 ` : ""
      }좋은 시간 보내야지${
        context.writingStyle.emojiUsage !== "minimal" ? " 😊" : ""
      }`,
      hashtags: [
  ...
  async getTodayContents(count: number = 3): Promise<PreparedContent[]> {
    try {
      ...
      // 개인화된 콘텐츠 생성
      const personalizedContents = await this.generatePersonalizedContents(
        context
      );
