...
class OpenRouterService {
  ...
  async generateContent(params: GenerateContentParams) {
    ...
    try {
      ...
      const cleanedLines = lines.filter((line) => {
        ...
        // 광고성 멘트 제거
        if (
          trimmedLine.includes("여러분은") &&
          trimmedLine.includes("어떻게")
        ) {
          return false;
        }
        if (
          trimmedLine.includes("저장하고") ||
          trimmedLine.includes("공유해")
        ) {
          return false;
        }
        if (
          trimmedLine.includes("댓글로") ||
          trimmedLine.includes("알려주세요")
        ) {
          return false;
        }
        if (trimmedLine.includes("맞팔") || trimmedLine.includes("소통해요")) {
          return false;
        }
  ...
  private createSystemPrompt(tone: string, platform?: string): string {
    const toneGuides = {
      casual:
        "친근하고 편안한 일상 대화체로 작성하세요. 이모지는 1-2개만 사용하세요.",
      professional:
        "깔끔하고 정돈된 느낌으로 작성하세요. 과하지 않게 적당히 진지하게.",
      humorous:
        "재미있고 가볍게 작성하세요. 억지 유머 말고 자연스러운 재치를 사용하세요.",
      emotional:
        "따뜻하고 감성적인 톤으로 작성하세요. 진심이 느껴지도록 하세요.",
    };
